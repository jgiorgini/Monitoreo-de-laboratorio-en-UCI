...
const STORAGE_KEY = 'uci_labs_registros_v10c';
...
function parseParametrosSimple(texto) {
  ...
  const listaAceptada = [
    ...
    'ferritina','troponina','ntprobnp','ggt','amilasa','lipasa','fosfato','fosforo'
  ];
  ...
  for (const linea of lineas) {
    ...
    else if (low.includes('fosfo') || low.includes('fosfato')) nombre = 'Fosfato';
...
function getParametrosPaciente(pac) {
  const set = new Set();
  for (const r of registros) {
    if (r.paciente === pac) {
      Object.keys(r.parametros).forEach(k => set.add(k));
...
function actualizarSelects() {
  ...
  const params = getParametrosGlobal();
...
function renderTablaUCI() {
  ...
  const params = getParametrosPaciente(pac);
